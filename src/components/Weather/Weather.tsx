...
const Weather = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async()=>{
            await axios.get(`https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=fc77e40ebafa7b3ca351479c35460584&units=metric`).then(response => {
  ...
  return (
    <>
    <Header />
    <div className="weatherMainContainer" style={{background: `url(${getBackgroundImage(weatherData?.weather?.[0]?.main)})`}}>
        <div className="weatherIconImg"><img src={`https://openweathermap.org/img/wn/${weatherData?.weather?.[0]?.icon}@2x.png`} alt="icon" />
        </div>
        <h1 className="weatherTempText">{weatherData?.main?.temp}° C</h1>
        <h4 className="weatherTempText2">{weatherData?.weather?.[0]?.description}</h4>
